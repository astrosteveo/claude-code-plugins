...
# Verifies that skills are resolved with correct priority: project > personal > agent-workflow
...
# 1. Create in agent-workflow location (lowest priority)
mkdir -p "$HOME/.config/opencode/agent-workflow/skills/priority-test"
cat > "$HOME/.config/opencode/agent-workflow/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Agent-workflow version of priority test skill
---
# Priority Test Skill (Agent-workflow Version)

This is the AGENT-WORKFLOW version of the priority test skill.

PRIORITY_MARKER_AGENT_WORKFLOW_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/agent-workflow/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Agent-workflow version exists"
else
    echo "  [FAIL] Agent-workflow version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides agent-workflow
echo ""
echo "Test 2: Testing personal > agent-workflow priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides agent-workflow)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_AGENT_WORKFLOW_VERSION"; then
    echo "  [FAIL] Agent-workflow version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|agent-workflow" | head -10
fi

# Test 3: Test that project overrides both personal and agent-workflow
echo ""
echo "Test 3: Testing project > personal > agent-workflow priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Agent-workflow version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit agent-workflow: prefix bypasses priority
echo ""
echo "Test 4: Testing agent-workflow: prefix forces agent-workflow version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load agent-workflow:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_AGENT_WORKFLOW_VERSION"; then
    echo "  [PASS] agent-workflow: prefix correctly forces agent-workflow version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] agent-workflow: prefix did not force agent-workflow version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
