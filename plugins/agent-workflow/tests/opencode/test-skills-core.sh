...
# Create skills in personal and agent-workflow locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/agent-workflow-skills/shared-skill"
mkdir -p "$TEST_HOME/agent-workflow-skills/unique-skill"
...
cat > "$TEST_HOME/agent-workflow-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: Agent-workflow version
---
# Agent-workflow Shared
EOF

cat > "$TEST_HOME/agent-workflow-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in agent-workflow
---
# Unique
EOF
...
function resolveSkillPath(skillName, agentWorkflowDir, personalDir) {
    const forceAgentWorkflow = skillName.startsWith('agent-workflow:');
    const actualSkillName = forceAgentWorkflow ? skillName.replace(/^agent-workflow:/, '') : skillName;

    if (!forceAgentWorkflow && personalDir) {
    ...
    if (agentWorkflowDir) {
        const agentWorkflowPath = path.join(agentWorkflowDir, actualSkillName);
        const agentWorkflowSkillFile = path.join(agentWorkflowPath, 'SKILL.md');
        if (fs.existsSync(agentWorkflowSkillFile)) {
            return {
                skillFile: agentWorkflowSkillFile,
...
const agentWorkflowDir = '$TEST_HOME/agent-workflow-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', agentWorkflowDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: agent-workflow: prefix should force agent-workflow
const forced = resolveSkillPath('agent-workflow:shared-skill', agentWorkflowDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to agent-workflow
const unique = resolveSkillPath('unique-skill', agentWorkflowDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', agentWorkflowDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow agent-workflow skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"agent-workflow"'; then
    echo "  [PASS] agent-workflow: prefix forces agent-workflow resolution"
else
    echo "  [FAIL] agent-workflow: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"agent-workflow"'; then
    echo "  [PASS] Unique agent-workflow skills are found"
else
    echo "  [FAIL] Unique agent-workflow skills not found"
    exit 1
fi
