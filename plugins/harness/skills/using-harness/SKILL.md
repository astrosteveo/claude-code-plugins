...
## Execute Phase: Subagent Orchestration

**Only the Execute phase uses subagent dispatch to parallelize implementation:**

The main agent orchestrates execution by:
1. **Baseline review** - Dispatch subagent to understand current state
2. **Implementation steps** - Dispatch subagent(s) for each step (parallel when independent)
3. **Progress tracking** - Coordinate completion and move to next steps

**Benefits:**
- Main context stays minimal during execution
- Independent steps run in parallel
- Each step has focused context from the detailed plan
- Faster implementation through parallelization
