...
description: Plugin validation specialist. Use proactively after creating or modifying plugin components, or when asked to validate plugin structure.
model: inherit
...
5. **Validate Agents** (if `agents/` exists):
   ...
   - For each agent file:
     ...
     - Or manually check:
       ...
       - Description is a concise single-line string (no XML tags, no `\n` escapes, no `<example>` blocks)
