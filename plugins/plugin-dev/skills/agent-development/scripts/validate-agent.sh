#!/bin/bash
# Agent File Validator
# Validates agent markdown files for correct structure and content
# Based on official Claude Code documentation

set -euo pipefail

# Usage
if [ $# -eq 0 ]; then
  echo "Usage: $0 <path/to/agent.md>"
  echo ""
  echo "Validates agent file for:"
  echo "  - YAML frontmatter structure"
  echo "  - Required fields (name, description)"
  echo "  - Optional fields (tools, model, permissionMode, skills)"
  echo "  - Description format (simple string, no XML or escapes)"
  echo "  - System prompt presence and quality"
  exit 1
fi

AGENT_FILE="$1"

echo "🔍 Validating agent file: $AGENT_FILE"
echo ""

# Check 1: File exists
if [ ! -f "$AGENT_FILE" ]; then
  echo "❌ File not found: $AGENT_FILE"
  exit 1
fi
echo "✅ File exists"

# Check 2: Starts with ---
FIRST_LINE=$(head -1 "$AGENT_FILE")
if [ "$FIRST_LINE" != "---" ]; then
  echo "❌ File must start with YAML frontmatter (---)"
  exit 1
fi
echo "✅ Starts with frontmatter"

# Check 3: Has closing ---
if ! tail -n +2 "$AGENT_FILE" | grep -q '^---$'; then
  echo "❌ Frontmatter not closed (missing second ---)"
  exit 1
fi
echo "✅ Frontmatter properly closed"

# Extract frontmatter and system prompt
FRONTMATTER=$(sed -n '/^---$/,/^---$/{ /^---$/d; p; }' "$AGENT_FILE")
SYSTEM_PROMPT=$(awk '/^---$/{i++; next} i>=2' "$AGENT_FILE")

# Check 4: Required fields
echo ""
echo "Checking fields..."

error_count=0
warning_count=0

# Check name field (REQUIRED)
NAME=$(echo "$FRONTMATTER" | grep '^name:' | sed 's/name: *//' | sed 's/^"\(.*\)"$/\1/')

if [ -z "$NAME" ]; then
  echo "❌ Missing required field: name"
  ((error_count++))
else
  echo "✅ name: $NAME"

  # Validate name format (lowercase, hyphens, numbers only)
  if ! [[ "$NAME" =~ ^[a-z0-9][a-z0-9-]*[a-z0-9]$ ]] && ! [[ "$NAME" =~ ^[a-z0-9]{1,2}$ ]]; then
    echo "❌ name must be lowercase letters, numbers, and hyphens only"
    ((error_count++))
  fi

  # Validate name length
  name_length=${#NAME}
  if [ $name_length -lt 2 ]; then
    echo "❌ name too short (minimum 2 characters)"
    ((error_count++))
  elif [ $name_length -gt 50 ]; then
    echo "❌ name too long (maximum 50 characters)"
    ((error_count++))
  fi

  # Check for generic names
  if [[ "$NAME" =~ ^(helper|assistant|agent|tool)$ ]]; then
    echo "⚠️  name is too generic: $NAME"
    ((warning_count++))
  fi
fi

# Check description field (REQUIRED)
DESCRIPTION=$(echo "$FRONTMATTER" | grep '^description:' | sed 's/description: *//')

if [ -z "$DESCRIPTION" ]; then
  echo "❌ Missing required field: description"
  ((error_count++))
else
  desc_length=${#DESCRIPTION}
  echo "✅ description: ${desc_length} characters"

  if [ $desc_length -lt 10 ]; then
    echo "⚠️  description too short (minimum 10 characters recommended)"
    ((warning_count++))
  elif [ $desc_length -gt 500 ]; then
    echo "⚠️  description is long (${desc_length} chars) - keep it concise (1-3 sentences)"
    ((warning_count++))
  fi

  # Check for forbidden patterns - these should NOT be in description
  if echo "$DESCRIPTION" | grep -q '<example>'; then
    echo "❌ description contains <example> tags - these should NOT be in the description field"
    ((error_count++))
  fi

  if echo "$DESCRIPTION" | grep -q '<commentary>'; then
    echo "❌ description contains <commentary> tags - these should NOT be in the description field"
    ((error_count++))
  fi

  if echo "$DESCRIPTION" | grep -q '\\n'; then
    echo "❌ description contains \\n escape sequences - use a simple single-line string"
    ((error_count++))
  fi

  # Check for proactive language (helpful tip)
  if echo "$DESCRIPTION" | grep -qi 'proactively\|immediately\|MUST BE USED'; then
    echo "💡 Good: description encourages proactive use"
  fi
fi

# Check model field (OPTIONAL - defaults to sonnet)
MODEL=$(echo "$FRONTMATTER" | grep '^model:' | sed 's/model: *//' || true)

if [ -n "$MODEL" ]; then
  echo "✅ model: $MODEL"

  case "$MODEL" in
    inherit|sonnet|opus|haiku)
      # Valid model
      ;;
    *)
      echo "⚠️  Unknown model: $MODEL (valid: inherit, sonnet, opus, haiku)"
      ((warning_count++))
      ;;
  esac
else
  echo "💡 model: not specified (defaults to sonnet)"
fi

# Check tools field (OPTIONAL - comma-separated string)
TOOLS=$(echo "$FRONTMATTER" | grep '^tools:' | sed 's/tools: *//' || true)

if [ -n "$TOOLS" ]; then
  echo "✅ tools: $TOOLS"

  # Warn if using JSON array format (wrong format)
  if echo "$TOOLS" | grep -q '^\['; then
    echo "⚠️  tools should be comma-separated (e.g., 'Read, Write, Grep'), not a JSON array"
    ((warning_count++))
  fi
else
  echo "💡 tools: not specified (inherits all tools from main thread)"
fi

# Check permissionMode field (OPTIONAL)
PERMISSION_MODE=$(echo "$FRONTMATTER" | grep '^permissionMode:' | sed 's/permissionMode: *//' || true)

if [ -n "$PERMISSION_MODE" ]; then
  echo "✅ permissionMode: $PERMISSION_MODE"

  case "$PERMISSION_MODE" in
    default|acceptEdits|bypassPermissions|plan|ignore)
      # Valid permissionMode
      ;;
    *)
      echo "⚠️  Unknown permissionMode: $PERMISSION_MODE (valid: default, acceptEdits, bypassPermissions, plan, ignore)"
      ((warning_count++))
      ;;
  esac
fi

# Check skills field (OPTIONAL)
SKILLS=$(echo "$FRONTMATTER" | grep '^skills:' | sed 's/skills: *//' || true)

if [ -n "$SKILLS" ]; then
  echo "✅ skills: $SKILLS"
fi

# Check color field (OPTIONAL)
COLOR=$(echo "$FRONTMATTER" | grep '^color:' | sed 's/color: *//' || true)
if [ -n "$COLOR" ]; then
  echo "✅ color: $COLOR"

  case "$COLOR" in
    blue|cyan|green|yellow|magenta|red)
      # Valid color
      ;;
    *)
      echo "⚠️  Unknown color: $COLOR (valid: blue, cyan, green, yellow, magenta, red)"
      ((warning_count++))
      ;;
  esac
fi

# Check 5: System prompt
echo ""
echo "Checking system prompt..."

if [ -z "$SYSTEM_PROMPT" ]; then
  echo "❌ System prompt is empty"
  ((error_count++))
else
  prompt_length=${#SYSTEM_PROMPT}
  echo "✅ System prompt: $prompt_length characters"

  if [ $prompt_length -lt 50 ]; then
    echo "⚠️  System prompt is very short (${prompt_length} chars) - consider adding more detail"
    ((warning_count++))
  elif [ $prompt_length -gt 10000 ]; then
    echo "⚠️  System prompt very long (over 10,000 characters)"
    ((warning_count++))
  fi

  # Check for second person (recommended)
  if echo "$SYSTEM_PROMPT" | grep -q "You are\|You will\|Your"; then
    echo "💡 Good: System prompt uses second person"
  else
    echo "⚠️  System prompt should use second person (You are..., You will...)"
    ((warning_count++))
  fi

  # Check for structure (helpful tips)
  if echo "$SYSTEM_PROMPT" | grep -qi "when invoked\|process\|steps\|checklist"; then
    echo "💡 Good: System prompt has clear structure"
  fi
fi

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

if [ $error_count -eq 0 ] && [ $warning_count -eq 0 ]; then
  echo "✅ All checks passed!"
  exit 0
elif [ $error_count -eq 0 ]; then
  echo "⚠️  Validation passed with $warning_count warning(s)"
  exit 0
else
  echo "❌ Validation failed with $error_count error(s) and $warning_count warning(s)"
  exit 1
fi
